...
5. 读取 [视觉系统](references/visual-system.md)。将配置中的人物图作为身份参考，用 imagegen 一次生成包含人物、日期卡和全部指定文字的完整创意底图；底部二维码与 Logo 区保持自然干净。
6. 用 `view_image` 逐字检查底图。日期、文字、人物、双图钉、人物标签或底部安全区任一失败，只针对失败项重生完整底图；最多两次自动返工。
